...
- **Модуль:** `agent/usage.js` → счётчики в `agent/data/usage.json`
- **Считает:** токены Groq (in + out), секунды Whisper, письма Brevo
- **Сброс:** автоматически в полночь (date-based)
- **Лимиты** (в `.env`):
  - `LIMIT_GROQ_TOKENS=2000000` — токенов/день
  - `LIMIT_WHISPER_SECONDS=28800` — секунд/день (8 часов)
  - `LIMIT_BREVO_EMAILS=300` — писем/день
- **Алерты:** при 80% и 100% лимита — в Telegram-бот
- **Просмотр:** `curl -s http://127.0.0.1:3001/health -H "x-agent-secret: $AGENT_HTTP_SECRET"` → поле `usage`

### Умный fallback

- **Первое сообщение + ошибка LLM:** `Здравствуйте! Прошу прощения за заминку…`
- **Середина диалога + ошибка LLM:** `Прошу прощения, техническая заминка — вернусь через минуту`
- **Retry:** перед fallback — попытка через `gpt-oss-20b` (быстрее, дешевле)

### Управление паузой (human takeover)### Управление паузой (human takeover)

- **Окно менеджера:** `MANAGER_ACTIVE_WINDOW_MS` в `.env` (по умолчанию **60 мин = 3600000 мс**)
- **Логика:**
  - Менеджер пишет вручную (Telegram или WhatsApp) → пауза на N минут
  - Каждое новое сообщение менеджера продлевает окно
  - Клиент пишет во время паузы → сообщение запоминается, но агент молчит
  - Клиент пишет после паузы → агент отвечает с учётом пропущенных сообщений
- **Синхронизация Telegram:** `syncTgHistory` перед каждым ответом подтягивает новые исходящие менеджера
- **Свой ответ в WhatsApp не сбивает:** `botSentTexts` трекер — бот отличает свои ответы от сообщений менеджера
- **Команды менеджера:** `/stop` (пауза до явного `/start`), `/start` (снять паузу), `/status`
